...
class WizardUpdateChartsAccounts(models.TransientModel):
    ...
    def action_init(self):
        ...
        self.write(
            {
                "state": "init",
                "account_group_ids": [(2, r.id, False) for r in self.account_group_ids],
    ...
    def _domain_tax_groups_to_deactivate(self, found_tax_groups_ids):
        return [
            ("company_id", "=", self.company_id.id),
            ("id", "not in", found_tax_groups_ids),
        ]
    ...
    def _find_tax_groups(self, t_data):
        ...
        # search for tax groups not in the template and propose them for
        # deactivation
        tax_groups_to_deactivate = self.env["account.tax.group"].search(
            self._domain_tax_groups_to_deactivate(found_tax_groups_ids)
        )
        for tax_group in tax_groups_to_deactivate:
            tax_group_vals.append(
                {
                    "update_chart_wizard_id": self.id,
                    "type": "deleted",
